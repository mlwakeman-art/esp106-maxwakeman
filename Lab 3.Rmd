...
setwd("/Users/howlermonkeymax/Desktop/esp106")
...
air1 <- air[air$Entity == country1, ]
air2 <- air[air$Entity == country2, ]


pm_col <- grep("particulate", names(air), value = TRUE)
oz_col <- grep("ozone", names(air), value = TRUE)
indoor_col <- grep("Household", names(air), value = TRUE)


air1$outdoor <- air1[[pm_col]] + air1[[oz_col]]
air2$outdoor <- air2[[pm_col]] + air2[[oz_col]]


ymax <- max(c(air1[[indoor_col]], air1$outdoor,
              air2[[indoor_col]], air2$outdoor),
            na.rm = TRUE)


plot(air1$Year, air1[[indoor_col]],
     type = "l",
     col = "blue",
     lty = 1,
     ylim = c(0, ymax),
...
lines(air1$Year, air1$outdoor, col = "blue", lty = 2)


lines(air2$Year, air2[[indoor_col]], col = "red", lty = 1)


lines(air2$Year, air2$outdoor, col = "red", lty = 2)


legend("topright",
       legend = c("India – Indoor", "India – Outdoor",
                  "USA – Indoor", "USA – Outdoor"),
...
merged <- merge(air, gdp, by = c("Entity", "Year"))
...
pm_col <- grep("particulate", names(merged), value = TRUE)
oz_col <- grep("ozone", names(merged), value = TRUE)
indoor_col <- grep("Household", names(merged), value = TRUE)
gdp_col <- grep("GDP", names(merged), value = TRUE)


merged$outdoor <- merged[[pm_col]] + merged[[oz_col]]


plotdata <- subset(merged,
                   merged[[gdp_col]] > 0 &
                   !is.na(merged[[indoor_col]]) &
                   !is.na(merged$outdoor))


ymax <- max(c(plotdata[[indoor_col]], plotdata$outdoor), na.rm = TRUE)


par(mfrow = c(1, 2))


plot(log(plotdata[[gdp_col]]),
     plotdata[[indoor_col]],
     pch = 16,
     ylim = c(0, ymax),
     xlab = "Log GDP per capita",
     ylab = "Deaths per 100,000",
     main = "Indoor Air Pollution")
